...
@interface AddPetViewController_iPhone5 ()

@end

@implementation AddPetViewController_iPhone5
...
@synthesize btnKennel;
@synthesize objPet;
@synthesize txtTitleKennel;
...
- (void) saveCurrentContacts:(ContactData *)contact
{
    ...
    if ([contact.type isEqualToString:kKennel])
    {
        ...
        [self.btnKennel setTitle:self.objPrimaryKennelData.name forState:UIControlStateNormal];
        [self.btnKennel setTitle:self.objPrimaryKennelData.name forState:UIControlStateHighlighted];
    }
}
...
- (void) setButtons
{
    ...
    self.btnKennel.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:12.f];
}
...
- (void) setFonts
{
    ...
    self.txtTitleKennel.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:12.0f];
...
- (void) savePet
{
    ...
    _imgIcon.alpha = 1.0f;
    _avatar.image = nil;
    //if (![self.btnBirthday.titleLabel.text isEqualToString:@"Tap to add birthday"]) self.objPetData.birthday = self.objBirthdayData.birthdayDate;
    
    //    BlockAlertView* confirm = [BlockAlertView alertWithTitle:@"Save" message:confirmMessage];
    //    [confirm setDestructiveButtonWithTitle:@"Confirm" block:^{
    //        [self.dataStack saveOrFail:^(NSError* error) {
    //
    //            //NSLog(@"%@, before vet count: %i", objPetData, [[Contact getContactData:kVeterinarian] count]);
    //
    //            NSLog(@"There was an error %@", error);
    //        }];
    //
    //        // NSLog(@"%@, after vet count: %i", objPetData, [[Contact getContactData:kVeterinarian] count]);
    //
    //        [[NSNotificationCenter defaultCenter] postNotificationName:@"savePet" object:nil];
    //        [self.parentViewController dismissModalViewControllerAnimated:YES];
    //    }];
    //
    //    [confirm show];
...
- (void) showGroomer
{
//    ContactListViewController* contactListViewController = [[ContactListViewController alloc] initWithNibName:@"ContactListViewController" bundle:nil withType:kGroomer andPet:objPetData displayAdd:YES];
...
//    ContactListViewController* contactListViewController = [[ContactListViewController alloc] initWithNibName:@"ContactListViewController" bundle:nil withType:kVeterinarian andPet:objPetData displayAdd:YES];
//    contactListViewController.delegate = self;
//    [self.navigationController pushViewController:contactListViewController animated:YES];
...
{
    [self checkTextFields];
    [self performSelector:@selector(showKennel) withObject:self afterDelay:0];
}

- (void) showKennel
{
//    ContactListViewController* contactListViewController = [[ContactListViewController alloc] initWithNibName:@"ContactListViewController" bundle:nil withType:kKennel andPet:objPetData displayAdd:YES];
//    contactListViewController.delegate = self;
//    [self.navigationController pushViewController:contactListViewController animated:YES];

    if ([[UIScreen mainScreen] bounds].size.height == 568)
    {
        AddContactViewController_iPhone5* addViewController = [[AddContactViewController_iPhone5 alloc] initWithNibName:@"AddContactViewController_iPhone5" bundle:nil withType:kKennel];
        UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:addViewController];
        navController.navigationBar.barStyle = UIBarStyleBlackOpaque;
        [self presentModalViewController:navController animated:YES];
    }
    ...
    {
        AddContactViewController* addViewController = [[AddContactViewController alloc] initWithNibName:@"AddContactViewController" bundle:nil withType:kKennel];
        UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:addViewController];
        navController.navigationBar.barStyle = UIBarStyleBlackOpaque;
        [self presentModalViewController:navController animated:YES];
    }
...
- (void) onSaveContact:(NSNotification *)note
{
    ...
    else if ([objContact.type isEqualToString:kKennel])
    {
        [self.btnKennel setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [self.btnKennel setTitle:objContact.name forState:UIControlStateNormal];
        [self.btnKennel setTitle:objContact.name forState:UIControlStateHighlighted];

        self.objPrimaryKennelData = objContact;
...
- (void) updatePetForm:(NSString *)type
{
    ...
    {
        [btnKennel setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [btnKennel setTitle:objContactData.name forState:UIControlStateNormal];
        [btnKennel setTitle:objContactData.name forState:UIControlStateHighlighted];
    }
}
