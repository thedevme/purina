...
@interface EditContactViewController_iPad ()

@end

@implementation EditContactViewController_iPad
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil andContactData:(ContactData *)contact
{
    ...
    if (self) {
        ...
        self.objContactData = contact;

        if ([self.objContactData.type isEqualToString:kVeterinarian])
        {
            self.navigationItem.title = @"Add A Veterinarian";
        }
        else if ([self.objContactData.type isEqualToString:kKennel])
        {
            self.navigationItem.title = @"Add A Kennel";
        }
        else if ([self.objContactData.type isEqualToString:kGroomer])
        {
            self.navigationItem.title = @"Add A Groomer";
        }
...
- (void)viewDidLoad
{
    ...
    [self updateContactForm:self.objContactData];
    NSLog(@"%@", self.objContactData.name);
}
...
- (void) addSaveBtn
{
    ...
    btnFind.frame = CGRectMake(100, 460, 307, 51);
...
- (void) createTitle:(NSString *)strTitle
{
    ...
    [attrStr setFont:[UIFont fontWithName:kHelveticaNeueCondBold size:45]];
...
- (void) saveContact
{
   // [self.navigationController dismissModalViewControllerAnimated:YES];
    ...
    [self.delegate saveContact:objContactData];
    if (!self.isEmergencyContact) [self.navigationController popViewControllerAnimated:YES];
    else [[self parentViewController] dismissModalViewControllerAnimated:YES];
}
...
{
    if (objContactData.name.length > 0)
    {
        [self saveContact];
    }
    else
    {
        UIAlertView* alert = [[UIAlertView alloc] initWithTitle:@"Sorry!"
                                                        message:@"Please enter contact information before saving."
                                                       delegate:self
...
{
    findContactView = [[FindContactViewController_iPad alloc] initWithNibName:@"FindContactViewController_iPad" bundle:nil withType:type showAdd:YES];
