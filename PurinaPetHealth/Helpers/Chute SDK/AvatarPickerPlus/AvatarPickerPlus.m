...
- (void)viewDidLoad
{
    ...
    UIImageView *tempImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ipadBackground.png"]];
    //[tempImageView setFrame:self.view.frame];
    [self.view addSubview:tempImageView];
    self.accounts = [[GCAccount sharedManager] accounts];

//    UIImageView *tempImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"background.png"]];
//    [tempImageView setFrame:self.accountsTable.frame];
//
//    self.accountsTable.backgroundView = tempImageView;
    NSLog(@"view did load account view controller");

    self.accountsTable = [[[UITableView alloc] initWithFrame:self.view.bounds style:UITableViewStylePlain] autorelease];
    self.accountsTable.separatorStyle = UITableViewCellSeparatorStyleNone;
...
{
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) return (interfaceOrientation == UIInterfaceOrientationPortrait);
...
-(void)viewDidLoad{
    ...
    UIImageView *tempImageView2 = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ipadBackground.png"]];
    //[tempImageView setFrame:self.view.frame];
    [self.view addSubview:tempImageView2];
    ...
    albumsTable.backgroundColor = [UIColor clearColor];
    
    UIImageView *tempImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ipadBackground.png"]];
    [tempImageView setFrame:self.albumsTable.frame];
    albumsTable.backgroundView = tempImageView;
    NSLog(@"view did load avatar picker");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        self.albumsTable.separatorStyle = UITableViewCellSeparatorStyleNone;
    }
    [cell.textLabel setText:@" "];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^(void) {
        UIView *v = [self tableView:tableView viewForIndexPath:indexPath];
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        [self setUseStandardDevicePicker:YES];
        [self setOfferLatestPhoto:NO];
        [self setTypePicked:NO];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    NSLog(@"view did load avatar controller");

    avatarTable.backgroundColor = [UIColor clearColor];
